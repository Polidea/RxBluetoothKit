...
public class RxCBPeripheral: RxPeripheralType {
    ...
    public var state: CBPeripheralState { return peripheral.state }
    public var services: [RxServiceType]? {
        guard let services = peripheral.services else { return nil }
        return services.map {RxCBService(service: $0) }
    }
    ...
    private class InternalPeripheralDelegate: NSObject, CBPeripheralDelegate {
        ...
        let peripheralDidReadRSSISubject = PublishSubject<(NSNumber, NSError?)>()
        ...
        @objc func peripheral(peripheral: CBPeripheral, didModifyServices invalidatedServices: [CBService]) {
            peripheralDidModifyServicesSubject.onNext(invalidatedServices.map { RxCBService(service: $0) })
        }
        
        
        @objc func peripheral(peripheral: CBPeripheral, didReadRSSI RSSI: NSNumber, error: NSError?) {
            peripheralDidReadRSSISubject.onNext((RSSI, error))
        }
        
        @objc func peripheral(peripheral: CBPeripheral, didDiscoverServices error: NSError?) {
            peripheralDidDiscoverServicesSubject.onNext((peripheral.services?.map{ RxCBService(service: $0) }, error))
        }

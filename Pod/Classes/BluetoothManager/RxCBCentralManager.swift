...
public class RxCBCentralManager : RxCentralManagerType {
    ...
    public var rx_didUpdateState: Observable<CBCentralManagerState> { return internalDelegate.didUpdateStateSubject }
    public var rx_willRestoreState: Observable<[String:AnyObject]> { return internalDelegate.willRestoreStateSubject }
    public var rx_didDiscoverPeripheral: Observable<(RxPeripheralType, [String : AnyObject], NSNumber)> { return internalDelegate.didDiscoverPeripheralSubject }
    public var rx_didConnectPeripheral: Observable<RxPeripheralType> { return internalDelegate.didConnectPerihperalSubject }
    public var rx_didFailToConnectPeripheral: Observable<(RxPeripheralType, NSError?)> { return internalDelegate.didFailToConnectPeripheralSubject }
    public var rx_didDisconnectPeripheral: Observable<(RxPeripheralType, NSError?)> { return internalDelegate.didDisconnectPeripheral }
    
    public var state: CBCentralManagerState { return centralManager.state }
    ...
    public func retrieveConnectedPeripheralsWithServices(serviceUUIDs: [CBUUID])  -> Observable<[RxPeripheralType]> {
        return Observable.just( centralManager.retrieveConnectedPeripheralsWithServices(serviceUUIDs).map { RxCBPeripheral(peripheral: $0) })
    }
    public func retrievePeripheralsWithIdentifiers(identifiers: [NSUUID]) -> Observable<[RxPeripheralType]> {
        return Observable.just(centralManager.retrievePeripheralsWithIdentifiers(identifiers).map { RxCBPeripheral(peripheral: $0) })
    }
}